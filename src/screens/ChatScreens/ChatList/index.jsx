...
/* --------------------- Chats Tab Component ------------------- */
function ChatsTab({ data, onOpenChat,navigation }) {
  const renderItem = (({ item }) => (
    <TouchableOpacity style={styles.row} activeOpacity={0.85} onPress={() => {navigation.navigate('openChat')}}>
...
export default function MessagesTabsScreen({ navigation }) {
  ...
  const openChat = React.useCallback(
    (user) => (navigation.getParent?.() ?? navigation).navigate('openChat', { user }),
    [navigation]
  );
  return (
